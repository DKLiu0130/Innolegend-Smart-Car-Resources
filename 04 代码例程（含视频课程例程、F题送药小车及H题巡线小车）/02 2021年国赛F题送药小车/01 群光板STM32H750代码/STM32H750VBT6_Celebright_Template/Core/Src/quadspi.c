...
int32_t QSPI_Status_Test ; 		 //检测标志位

uint32_t W25Qxx_TestAddr  =	0	;							// 测试地址
uint8_t  W25Qxx_WriteBuffer_Test[W25Qxx_NumByteToTest];		//	写数据数组
uint8_t  W25Qxx_ReadBuffer_Test[W25Qxx_NumByteToTest];		//	读数据数组
...
// 擦除 >>>>>>>    
	...
	QSPI_Status_Test 			= QSPI_W25Qxx_BlockErase_32K(W25Qxx_TestAddr);	// 擦除32K字节
...
// 写入 >>>>>>>    
	...
	ExecutionTime_Begin 	= HAL_GetTick();	// 获取 systick 当前时间，单位ms
	QSPI_Status_Test				= QSPI_W25Qxx_WriteBuffer(W25Qxx_WriteBuffer_Test,W25Qxx_TestAddr,W25Qxx_NumByteToTest); // 写入数据
	ExecutionTime_End		= HAL_GetTick();	// 获取 systick 当前时间，单位ms
...
// 读取 >>>>>>>   
	...
	printf ("\r\n*****************************************************************************************************\r\n");				
	ExecutionTime_Begin 	= HAL_GetTick();	// 获取 systick 当前时间，单位ms	
	QSPI_Status_Test				= QSPI_W25Qxx_ReadBuffer(W25Qxx_ReadBuffer_Test,W25Qxx_TestAddr,W25Qxx_NumByteToTest);	// 读取数据
	ExecutionTime_End		= HAL_GetTick();	// 获取 systick 当前时间，单位ms
...
// 数据校验 >>>>>>>    
	...
	{
		if( W25Qxx_WriteBuffer_Test[i] != W25Qxx_ReadBuffer_Test[i] )	//如果数据不相等，则返回0	
		{
			printf ("\r\n数据校验失败!!!!!\r\n");	
			while(1);
		}
