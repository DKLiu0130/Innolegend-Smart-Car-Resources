...
extern QueueHandle_t uart5_frame_queue;  // 由主程序创建
...
static uint8_t rx5_frame_buf[UART_FRAME_MAX_LEN];
static size_t frame5_idx = 0;
static uint8_t rx5_byte;
...
void uart_frame_rx_init(void)
{
    ...
    // UART5初始化（摄像头通信）
    frame5_idx = 0;
    memset(rx5_frame_buf, 0, UART_FRAME_MAX_LEN);
    HAL_UART_Receive_IT(&huart5, &rx5_byte, 1);//camera
}
//以上为bt和cat,3cat4bt
//******************//
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart) {
    ...
    if (huart->Instance == UART5) {
        ...
        if (rx5_byte == 0xFE) {  // 帧头
            frame5_idx = 0;
            rx5_frame_buf[frame5_idx++] = rx5_byte;
        } else if (frame5_idx > 0) {  // 已经接收到帧头
            if (rx5_byte == 0xFD) {  // 帧尾
                rx5_frame_buf[frame5_idx++] = rx5_byte;
                // 发送完整帧到队列
                BaseType_t xHigherPriorityTaskWoken = pdFALSE;
                xQueueSendFromISR(uart5_frame_queue, rx5_frame_buf, &xHigherPriorityTaskWoken);
                frame5_idx = 0;
                memset(rx5_frame_buf, 0, UART_FRAME_MAX_LEN);
                portYIELD_FROM_ISR(xHigherPriorityTaskWoken);
            } else if (frame5_idx < UART_FRAME_MAX_LEN - 1) {
                rx5_frame_buf[frame5_idx++] = rx5_byte;
            }
        }
        // 继续接收下一个字节
        HAL_UART_Receive_IT(&huart5, &rx5_byte, 1);
    }
}
...
void UART5_SendPosition(uint8_t position) {
    uint8_t frame[3];
    frame[0] = 0xFE;  // 帧头
    frame[1] = (0x00 << 4) | (position & 0x0F);  // 前4位0000，后4位是位置
    frame[2] = 0xFD;  // 帧尾
    
    HAL_UART_Transmit(&huart5, frame, 3, 100);  // 阻塞发送
}
