...
#define REMOTE_SERVICE_UUID        0xA001
#define REMOTE_NOTIFY_CHAR_UUID    0xA002
...
static char remote_device_name[ESP_BLE_ADV_NAME_LEN_MAX] = "ESP32_Drone";
static bool connect    = false;
static bool get_server = false;
static esp_gattc_char_elem_t *char_elem_result   = NULL;
static esp_gattc_descr_elem_t *descr_elem_result = NULL;
static volatile bool ble_write_ready = true;
...
void uart_init(void)
{
    ...
    // TXD: GPIO2, RXD: GPIO4
    uart_set_pin(UART_NUM, 2, 4, UART_PIN_NO_CHANGE, UART_PIN_NO_CHANGE);
}
...
static void gattc_profile_event_handler(esp_gattc_cb_event_t event, esp_gatt_if_t gattc_if, esp_ble_gattc_cb_param_t *param)
{
    ...
    case ESP_GATTC_CONNECT_EVT:{
        ...
        gpio_set_level(LED_Pin, 1); // Turn on the LED to indicate connection
        if (mtu_ret){
    ...
    case ESP_GATTC_OPEN_EVT:
        if (param->open.status != ESP_GATT_OK){
            ...
            esp_ble_gap_start_scanning(0);        // 重新扫描
            break;
        }
    ...
    case ESP_GATTC_NOTIFY_EVT:
        ...
        // 新增：以字符串形式输出收到的数据
        char msg[64] = {0};
        int len = p_data->notify.value_len > 63 ? 63 : p_data->notify.value_len;
        memcpy(msg, p_data->notify.value, len);
        ESP_LOGI(GATTC_TAG, "Received message from slave: %s", msg);

        // 新增：通过 UART1 发送收到的数据（原始字节流）
        uart_write_bytes(UART_NUM, (const char *)p_data->notify.value, p_data->notify.value_len);
        break;
    ...
    case ESP_GATTC_DISCONNECT_EVT:
        ...
        gpio_set_level(LED_Pin, 0); // Turn off the LED to indicate disconnection
        vTaskDelay(500 / portTICK_PERIOD_MS); // 延时
        esp_ble_gap_start_scanning(0);        // 重新扫描
        break;
    default:
...
void uart_rx_task(void *arg)
{
    ...
    while (1) {
        int len = uart_read_bytes(UART_NUM, rx_buf, BUF_SIZE - 1, pdMS_TO_TICKS(100));
        if (len > 0) {
            // 查找第一个\n和第二个\n
            int first_nl = -1, second_nl = -1;
            for (int i = 0; i < len; ++i) {
                if (rx_buf[i] == '\n') {
                    if (first_nl < 0) {
                        first_nl = i;
            ...
            if (first_nl >= 0 && second_nl > first_nl) {
                int slice_len = second_nl - first_nl;
                if (slice_len > FRAME_BUF_SIZE - 1)
                    slice_len = FRAME_BUF_SIZE - 1;
                memcpy(frame_buf, rx_buf + first_nl + 1, slice_len); // 包含第二个\n
                frame_len = slice_len;
                // 发送BLE
                if (gl_profile_tab[PROFILE_A_APP_ID].char_handle != INVALID_HANDLE && connect) {
                    ...
                    //ESP_LOGI(GATTC_TAG, "Forwarded UART 2nd frame to BLE: %.*s", frame_len, frame_buf);
                } else {
                    ESP_LOGW(GATTC_TAG, "BLE not connected or char handle invalid");
                }
            }
            // 其它情况（不足两个\n）不发
        }
...
void app_main(void)
{
    ...
    */
    ...
    uint8_t addr_type = 0;
    esp_err_t addr_ret = esp_ble_gap_get_local_used_addr(bd_addr, &addr_type);
    if (addr_ret == ESP_OK) {
        ESP_LOGI(GATTC_TAG, "Local BLE address: %02x:%02x:%02x:%02x:%02x:%02x (type: %d)",
                 bd_addr[0], bd_addr[1], bd_addr[2], bd_addr[3], bd_addr[4], bd_addr[5], addr_type);
    } else {
        ESP_LOGE(GATTC_TAG, "Failed to get local BLE address, error code = %x", addr_ret);
    }
